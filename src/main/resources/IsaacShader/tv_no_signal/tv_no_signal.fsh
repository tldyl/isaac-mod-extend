...
void main() {
    ...
    flooredTexCoord = floor(flooredTexCoord / 0.003) * 0.003;

    vec3 mixColor = vec3(random(vec2((flooredTexCoord.x + duration) / 2.0, (flooredTexCoord.y + duration) / 2.0)));
    vec3 finalColor = mixColor.rgb * mix_level + texColor.rgb * (1.0 - mix_level);
